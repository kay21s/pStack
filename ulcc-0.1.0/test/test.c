...
void test1()
{
	...
	end = (char *)ULCC_ALIGN_LOWER((unsigned long)(mem + size));
	...
	cs.s_size = 1024 * 1024 * 11;
	aid_strong = cc_alloc2((unsigned long)start,
		(unsigned long)(start + 1024 * 1024 * 10), NULL, &cs, 0);
	...
	cs.s_type = CC_SHARED;
	cs.s_size = 1024 * 512;
	aid_weak = cc_alloc2_cpus((unsigned long)(start + 1024 * 1024 * 10),
...
void test2()
{
	int				size1 = 4 * 2 * 1024 * 1024, size2 = 16 * 1024 * 1024;
	pthread_t		tid[2] = {-1, -1};
	void			*data1, *data2;
...
void test3()
{
	...
	if(cc_alloc_add2(aid_strong, (unsigned long)start,
		(unsigned long)(start + 4096 * 640 * 4), 0) != aid_strong)
	{
		printf("cc_alloc_add2 error for strong locality data");
	}
	if(cc_alloc_add2(aid_weak, (unsigned long)(start + 4096 * 640 * 4),
