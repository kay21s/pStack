...
#if 1
#define SET_NUMBER 200000 //0.1 Million buckets = 1.4 Million Elem
...
tcp_init(int size, TCP_THREAD_LOCAL_P tcp_thread_local_p)
{
	...
	core_elem_num = MAX_STREAM/(number_of_cpus_used - 1);
	...
	cc_cacheregn_set(&regn, 150 * color_range, 150 * (color_range + 1), 1);
	...
	printf("ULCC Allocation: Core %d, Color range %d ~ %d, size = %d.\n", 
		tcp_thread_local_p->self_cpu_id, 
		50 * color_range, 50 * (color_range + 1),
		64 * tcp_thread_local_p->tcp_stream_table_size);
#endif
