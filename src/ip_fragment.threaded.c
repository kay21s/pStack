...
static int
//! jiffies()
jiffies(IP_THREAD_LOCAL_P  ip_thread_local_p)
{
	struct timeval tv;

//!   if (timenow)
	if (ip_thread_local_p->timenow)
//!     return timenow;
		return ip_thread_local_p->timenow;
	gettimeofday(&tv, 0);
//!   timenow = (tv.tv_sec - time0) * 1000 + tv.tv_usec / 1000;
	ip_thread_local_p->timenow = (tv.tv_sec - ip_thread_local_p->time0) * 1000 + tv.tv_usec / 1000;
...
add_timer(struct timer_list * x,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
//!   if (timer_tail) {
	if (ip_thread_local_p->timer_tail) {
//!     timer_tail->next = x;
		ip_thread_local_p->timer_tail->next = x;
//!     x->prev = timer_tail;
		x->prev = ip_thread_local_p->timer_tail;
		x->next = 0;
//!     timer_tail = x;
		ip_thread_local_p->timer_tail = x;
	} else {
		x->prev = 0;
		x->next = 0;
//!     timer_tail = timer_head = x;
		ip_thread_local_p->timer_tail = ip_thread_local_p->timer_head = x;
	}
}

static void
//! del_timer(struct timer_list * x)
del_timer(struct timer_list * x,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
	if (x->prev)
		x->prev->next = x->next;
	else
//!     timer_head = x->next;
		ip_thread_local_p->timer_head = x->next;
	if (x->next)
		x->next->prev = x->prev;
	else
//!     timer_tail = x->prev;
		ip_thread_local_p->timer_tail = x->prev;
}

static void
//! frag_kfree_skb(struct sk_buff * skb, int type)
frag_kfree_skb(struct sk_buff * skb, int type,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
//!   if (this_host)
	if (ip_thread_local_p->this_host)
//!     atomic_sub(skb->truesize, &this_host->ip_frag_mem);
		atomic_sub(skb->truesize, &ip_thread_local_p->this_host->ip_frag_mem);
	kfree_skb(skb, type);
}

static void
//! frag_kfree_s(void *ptr, int len)
frag_kfree_s(void *ptr, int len,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
//!   if (this_host)
	if (ip_thread_local_p->this_host)
//!     atomic_sub(len, &this_host->ip_frag_mem);
		atomic_sub(len, &ip_thread_local_p->this_host->ip_frag_mem);
	free(ptr);
}

static void *
//! frag_kmalloc(int size, int dummy)
frag_kmalloc(int size, int dummy,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
...
//!   atomic_add(size, &this_host->ip_frag_mem);
	atomic_add(size, &ip_thread_local_p->this_host->ip_frag_mem);
...
//! ip_frag_create(int offset, int end, struct sk_buff * skb, unsigned char *ptr)
			ip_frag_create(int offset, int end, struct sk_buff * skb, unsigned char *ptr,IP_THREAD_LOCAL_P  ip_thread_local_p) {
	struct ipfrag *fp;

//!   fp = (struct ipfrag *) frag_kmalloc(sizeof(struct ipfrag), GFP_ATOMIC);
	fp = (struct ipfrag *) frag_kmalloc(sizeof(struct ipfrag), GFP_ATOMIC,ip_thread_local_p);
	if (fp == NULL) {
...
//!   this_host->ip_frag_mem += skb->truesize;
	ip_thread_local_p->this_host->ip_frag_mem += skb->truesize;
...
frag_index(struct ip * iph,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
	unsigned int ip = ntohl(iph->ip_dst.s_addr);

//!   return (ip % hash_size);
	return (ip % ip_thread_local_p->hash_size);
}
...
static void
//! rmthis_host()
rmthis_host(IP_THREAD_LOCAL_P  ip_thread_local_p)
{
//!   int hash_index = this_host->hash_index;
	int hash_index = ip_thread_local_p->this_host->hash_index;

//!   if (this_host->prev) {
	if (ip_thread_local_p->this_host->prev) {
//!     this_host->prev->next = this_host->next;
		ip_thread_local_p->this_host->prev->next = ip_thread_local_p->this_host->next;
//!     if (this_host->next)
		if (ip_thread_local_p->this_host->next)
//!       this_host->next->prev = this_host->prev;
			ip_thread_local_p->this_host->next->prev = ip_thread_local_p->this_host->prev;
	} else {
//!     fragtable[hash_index] = this_host->next;
		ip_thread_local_p->fragtable[hash_index] = ip_thread_local_p->this_host->next;
//!     if (this_host->next)
		if (ip_thread_local_p->this_host->next)
//!       this_host->next->prev = 0;
			ip_thread_local_p->this_host->next->prev = NULL;
	}
//!   free(this_host);
	free(ip_thread_local_p->this_host);
//!   this_host = 0;
	ip_thread_local_p->this_host = NULL;
}
...
//! ip_find(struct ip * iph)
			ip_find(struct ip * iph,IP_THREAD_LOCAL_P  ip_thread_local_p) {
	struct ipq *qp;
	struct ipq *qplast;

	qplast = NULL;
//!   for (qp = this_host->ipqueue; qp != NULL; qplast = qp, qp = qp->next) {
	for (qp = ip_thread_local_p->this_host->ipqueue; qp != NULL; qplast = qp, qp = qp->next) {
...
//!       del_timer(&qp->timer);	/* So it doesn't vanish on us. The timer will
			del_timer(&qp->timer,ip_thread_local_p);	/* So it doesn't vanish on us. The timer will
				   be reset anyway */
			return (qp);
		}
	}
...
ip_defrag(struct ip *iph, struct sk_buff *skb,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
	...
	if (!hostfrag_find(iph, ip_thread_local_p) && skb)
//!     hostfrag_create(iph);
		hostfrag_create(iph,ip_thread_local_p);

	/* Start by cleaning up the memory. */
//!   if (this_host)
	if (ip_thread_local_p->this_host)
//!     if (this_host->ip_frag_mem > IPFRAG_HIGH_THRESH)
		if (ip_thread_local_p->this_host->ip_frag_mem > IPFRAG_HIGH_THRESH)
//!       ip_evictor();
			ip_evictor(ip_thread_local_p);

	/* Find the entry of this IP datagram in the "incomplete datagrams" queue. */
//!   if (this_host)
	if (ip_thread_local_p->this_host)
//!     qp = ip_find(iph);
		qp = ip_find(iph,ip_thread_local_p);
	else
		qp = NULL;
...
//!       ip_free(qp);		/* Fragmented frame replaced by full
			ip_free(qp,ip_thread_local_p);		/* Fragmented frame replaced by full
				   unfragmented copy */
		return 0;
	}
...
//    printf(" use ip_create \n");
		if ((qp = ip_create(iph,ip_thread_local_p)) == NULL) {
...
//!       frag_kfree_skb(tmp->skb, FREE_READ);
			frag_kfree_skb(tmp->skb, FREE_READ,ip_thread_local_p);
//!       frag_kfree_s(tmp, sizeof(struct ipfrag));
			frag_kfree_s(tmp, sizeof(struct ipfrag),ip_thread_local_p);
		}
	}
	/* Insert this fragment in the chain of fragments. */
	tfp = NULL;
//!   tfp = ip_frag_create(offset, end, skb, ptr);
	tfp = ip_frag_create(offset, end, skb, ptr,ip_thread_local_p);
...
//!     skb2 = ip_glue(qp);		/* glue together the fragments */
		skb2 = ip_glue(qp,ip_thread_local_p);		/* glue together the fragments */
		return (skb2);
	}
	return (NULL);
}
...
void
//! ip_frag_init(int n)
ip_frag_init(int n,IP_THREAD_LOCAL_P  ip_thread_local_p)
{
	struct timeval tv;

	gettimeofday(&tv, 0);
	ip_thread_local_p->time0 = tv.tv_sec;
//!   fragtable = (struct hostfrags **) calloc(n, sizeof(struct hostfrags *));
	ip_thread_local_p->fragtable = (struct hostfrags **) calloc(n, sizeof(struct hostfrags *));
//!   if (!fragtable)
	if (!ip_thread_local_p->fragtable)
		nids_params.no_mem("ip_frag_init");
//!   hash_size = n;
	ip_thread_local_p->hash_size = n;
}

void
//! ip_frag_exit(void)
ip_frag_exit(IP_THREAD_LOCAL_P  ip_thread_local_p)
{
//!   if (fragtable) {
	if (ip_thread_local_p->fragtable) {
//!     free(fragtable);
		free(ip_thread_local_p->fragtable);
//!     fragtable = NULL;
		ip_thread_local_p->fragtable = NULL;
	}
