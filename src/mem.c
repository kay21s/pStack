...
/*
#if defined(MEM_ALIGN)
	if (0 != posix_memalign((void**)&mem_table, 64, CPU_NUM * sizeof(char **))) {
		printf("memalign allocation failed, exit\n");
		exit(0);
	}
	
	for (i = 0; i < CPU_NUM; i ++) {
		if (0 != posix_memalign((void**)&(mem_table[i]), 64, size_num * sizeof(char *))) {
			printf("memalign allocation failed, exit\n");
			exit(0);
		}
	}
...
//	size_table = malloc(size_num * sizeof(unsigned short));
//	num_table = malloc(size_num * sizeof(unsigned short));
...
*********************************************************/
void mem_init(int size_type, int block_num, int size, int cpu_id)
{
	int i;
...
*********************************************************/
void *mem_alloc(int size_type, int cpu_id)
{
	void *p_block;
	if (mem_table[cpu_id][size_type] == NULL)
...
*********************************************************/
void mem_free(void *p_block, int size_type, int cpu_id)
{
